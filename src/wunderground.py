...
#                    return data_condition[location]
    ...
    # URL에 location 삽입
    if is_korean:
        url = url_conditions_korea.format(location)
    else:
        url = url_conditions_not_korea.format(location)

    # 콜 지연에 sleep() 사용

    # 1분에 10콜
    if not call_per_minute.full():
        pass
    else:
