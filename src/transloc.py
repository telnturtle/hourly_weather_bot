...
def translate(location):
    ...
    if not is_korean(location): return location
    elif location in LOOKUP_TABLE: return LOOKUP_TABLE[location]
    else:
        _si_gun = si_gun(location)
        if _si_gun:
            if _si_gun in LOOKUP_TABLE: return LOOKUP_TABLE[_si_gun]
            elif _si_gun in REDIRECTS:
                if REDIRECTS[_si_gun] in LOOKUP_TABLE: return LOOKUP_TABLE[REDIRECTS[_si_gun]]
                else: return None
            else: return None
        else: return None
